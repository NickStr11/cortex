...
| Ретроспективный вопрос юзера («когда мы решали X», «что было с Y», «помнишь как…») | Сначала `python scripts/session-search.py "<query>"`, потом отвечать |
| Production-баг / странный симптом («сломалось», «не работает», «опять то же что было») | Сначала `grep -i "<symptom>" docs/learned-the-hard-way.md` — recipe может быть готов. Если нет — после фикса добавить новую запись (формат: симптом / причина / фикс с командой / источник) |
| Исследование внешнего репо/проекта | Скопировать `.claude/templates/recon-report.md` в `runtime/research/<name>-recon-YYYY-MM-DD.md` и заполнить по полям (не писать в свободной форме) |
| После кода | Авто (pre-commit hook) |
| Значимые изменения | `/verify` |
| `/handoff` если прошло 7+ дней с прошлого `scripts/scan-skills.py` (есть `runtime/cache/last-skill-scan.txt`) | Прогнать `scan-skills`, сохранить отчёт в `runtime/skill-scans/YYYY-MM-DD.md`, обновить timestamp |
...
- `docs/learned-the-hard-way.md` — lookup-recipes по симптомам production-багов
