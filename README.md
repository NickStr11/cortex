...
- **[`scripts/session-search.py`](scripts/session-search.py)** — SQLite FTS5 поверх diary
...
Сессия → /diary в per-user folder
       ...
       ↓
       Правило автоматически грузится в каждый новый чат
```
...
| `tg-digest-reader` | Sonnet | TG-дайджесты с GCP VM |
...
- **Hermes Agent** (NousResearch, [github.com/NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)) — изучен через recon-report. Взяли идеи: SQLite FTS5 session search, read-only skill curator, dry-run для миграций, recon-template. Отвергли: runtime, gateway, autonomous skill mutation, plaintext SUDO_PASSWORD pattern.
